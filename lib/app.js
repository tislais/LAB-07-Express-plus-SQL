  ...
  try {
    ...
    const data = await client.query(`
      ...
      RETURNING 
        ...
        is_favorite as "isFavorite",
        user_id as "userId";
    `, [machine.title, machine.manufacturer, machine.dateOfManufacture, machine.unitsProduced, machine.abbreviation, machine.mpu, machine.type, machine.designer, machine.image, machine.manual, machine.funRating, machine.isFavorite, 1]);
    res.json(data.rows[0]);
  }
  catch(err) {
    console.log(err);
